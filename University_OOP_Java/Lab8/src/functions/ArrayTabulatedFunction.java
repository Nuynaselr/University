...
public class ArrayTabulatedFunction implements TabulatedFunction, Serializable, Cloneable{
    ...
    public ArrayTabulatedFunction(double leftX, double rightX, double[] values) throws InappropriateFunctionPointException {
        ...
        this.length = countPoint;
        for (int i = 0; i < countPoint; i++){
            this.arrayPoint[i] = new FunctionPoint(leftX + i * lengthBePoint, values[i]);
        }
    }
    ...
    public Iterator<FunctionPoint> iterator() {
        return new Iterator<>() {
            ...
            public boolean hasNext() {
                return ((i + 1) < length) && (arrayPoint[i + 1] != null);
            }

            @Override
            public FunctionPoint next() {
                if (hasNext())
                    return new FunctionPoint(arrayPoint[++i]);
                else
