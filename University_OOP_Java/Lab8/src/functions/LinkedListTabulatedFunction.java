...
public class LinkedListTabulatedFunction implements TabulatedFunction, Serializable, Cloneable {
    ...
    public LinkedListTabulatedFunction(FunctionPoint[] array) throws InappropriateFunctionPointException {
        ...
        this.head = new FunctionNode(array[0]);
        this.tail = new FunctionNode(array[array.length - 1]);
        connectionHeadTail(this.head, this.tail);
    ...
    public Iterator<FunctionPoint> iterator() {
        return new Iterator<>() {
            private FunctionNode current = head.prev;
            private boolean virgin = true;

            @Override
            public boolean hasNext() {
                return (current != head.prev || virgin) && (current.next.value != null);
            }
            ...
            public FunctionPoint next() {
                if (hasNext()) {
                    ...
                    return new FunctionPoint(current.value);
                } else
